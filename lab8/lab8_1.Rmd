...
```{r}
#always puts you in the main part
here()
```
...
```{r}
#different drugs are data, so every variable does not have its own column
heartrate
```
...
```{r}
#column names after patient are a problem, plotting only works on tidy data
heartrate %>% 
  pivot_longer(-patient, #patient will not pivot
               names_to = "drug", #make a new column name
               values_to = "heartrate"
               ) #what values will be moved to
```
...
# the incomes dont have their own columns
...
# the days do not have their own columns
