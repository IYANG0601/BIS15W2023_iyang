...
```r
#I removed the here thing because for some reason it would not let me knit even after I downloaded the Rtools thing it told me to install.
```
...
```
## here() starts at C:/Users/isaac/OneDrive/Desktop/BIS15W2023_iyang
```
...
```r
#The data is tidy as each variable has its own column, each observation has its own row, and each value has its own cell. There is no weird combinations or anything lacking a label. They are currently in a long format as each column is about a single variable.
sydneybeaches
```
...
sydneybeaches_long %>% 
  ...
  group_by(site, year) %>% 
  summarize(enterrococci_cfu_100ml_avg =mean(enterococci_cfu_100ml, na.rm=T))
```
...
## # A tibble: 66 × 3
## # Groups:   site [11]
##    site         year  enterrococci_cfu_100ml_avg
##    <chr>        <chr>                      <dbl>
...
sydneybeaches_new <- sydneybeaches_long %>% 
  ...
  pivot_wider(names_from = "year",
              values_from = "enterrococci_cfu_100ml_avg")
```
...
```r
#The most polluted beach is South Maroubra Rockpool
sydneybeaches_new %>% 
  select("site", "2018")
```
