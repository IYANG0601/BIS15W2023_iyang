...
```{r}
my_palette <- paletteer_d("ggprism::flames") #package::palette
```
...
```{r}
p+scale_fill_manual(values=my_palette)+ #use my colors
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 60, hjust=1))+
  labs(title = "Observations by Taxon in Homerange Data",
       x = NULL,
...
homerange %>% 
  ...
  ggplot(aes(x=trophic.guild, fill = class))+geom_bar(position = "dodge")+
...
```{r}
#very messy and hard to interpret
ecosphere %>% 
...
ecosphere %>% 
  ggplot(aes(x=diet, y=log10_mass, fill=migratory_strategy))+ 
  geom_boxplot(alpha=0.4) + 
  facet_wrap(~migratory_strategy, ncol=4)+ #do it by a variable and state number of columns
  theme(axis.text.x = element_text(angle = 60, hjust = 1))+
...
rows~columns
```{r}
#control over the grouped variable in rows or columns
ecosphere %>% 
  ggplot(aes(x=diet, y=log10_mass, fill=migratory_strategy))+ 
  geom_boxplot(alpha=0.4)+ 
...
1. Build a faceted plot that shows a distribution of log10.mas by habitat type.
```{r}
ecosphere %>% 
  ggplot(aes(x= log10_mass, fill = habitat))+geom_density()+
  facet_wrap(~habitat)
```

2. Try the same plot as #1 above, but include life_expectancy~habitat.
```{r}
#extension of grouping
ecosphere %>% 
  ggplot(aes(x= log10_mass, fill = habitat))+geom_density()+
  facet_wrap(habitat~life_expectancy)
```
