...
```{r}
homerange %>% 
  ggplot(aes(x= class, y= log10.mass, fill = taxon))+geom_col()+
  labs(title = "Class Mass",
       x= "Class",
       y= "Log 10 Mass")
```
...
```{r}
deserts2 <- deserts %>% mutate(year=as_factor(year)) #change them from numeric to factor so that it doesnt get grouped up
```
...
deserts2 %>% 
  ...
  geom_line()+
  geom_point(shape=5)+ #add a point layer to give the edges points
...
```{r}
homerange %>% 
  ggplot(aes(x = log10.mass)) + #color is the boundary around it
  geom_histogram(alpha = 9, color = "white", fill = "ivory", bins = 40)+ #alpha for transparency, tell the color 
...
homerange %>% 
  ...
  geom_density(fill="deepskyblue3", alpha  =0.4, color = "black")+
  labs(title = "Distribution of Body Mass")
```
...
homerange %>% 
  ggplot(aes(x= log10.hra))+
  geom_histogram(aes(y= after_stat(density)), color = "black", fill = "blue", alpha = 0.2)+
  geom_density(color = "red")+
  labs(title = "Distribution of Homerange",
       x= "Log.10 HRA",
       y= "Density")
...
homerange <- homerange %>% 
  mutate(range_category = case_when(log10.hra < 3.65 ~ "small",
                                              log10.hra > 3.65 & log10.mass <= 4.59 ~ "medium",
                                              log10.hra > 4.59 & log10.mass <= 6.02 ~ "large",
                                              log10.hra > 6.02 ~ "extra large"), na.rm = T)
...
```{r}
homerange %>% 
  ggplot(aes(x= class, y = range_category))+geom_col()
```
...
homerange %>% 
  ...
  ggplot(aes(x= range_category, y= log10.mass, fill = class))+geom_boxplot()
```
