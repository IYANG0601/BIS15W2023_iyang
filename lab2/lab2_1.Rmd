...
This result is due to the missing value. R can't calculate the mean if it does not know one of the potential values. This means that NA prevents some calculations, even if there are many given values and only one NA.

4. Recalculate the mean using the following code chunk. Why is the useful? 
It helps us calculate mean and other functions without NA interering.
```{r}
