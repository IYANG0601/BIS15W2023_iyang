...
ecosphere %>% 
  group_by(habitat) %>% 
  summarize(habitat_total=sum(scientific_name, na.rm=T))
...
ducks %>% 
  select(common_name, scientific_name, diet, life_expectancy, habitat, urban_affiliate, migratory_strategy,log10_mass, mean_eggs_per_clutch, mean_age_at_sexual_maturity, population_size, winter_range_area, range_in_cbc, strata, circles, feeder_bird, median_trend, lower_95_percent_ci, upper_95_percent_ci)
```
...
```{r}
#Ducks migrating long distance have a somewhat low body mass.
ducks %>% 
...
eagles %>% 
  select(common_name, scientific_name, population_size) %>% 
  mutate(conservation_status = population_size < 250,000) 
```
