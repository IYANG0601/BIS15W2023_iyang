...
```{r}
life_history %>% 
  count(order, sort=T) #sort = T is another way to arrange
```
...
```
```{r}
#options(scipen = 999) #cancels scientific notation for the whole session
```
...
life_history %>% 
  ...
  coord_flip()+
  scale_y_log10() #use the scale to modify
```
...
life_history %>% 
  group_by(order) %>% 
  summarize(mean_mass=mean(mass, na.rm=T)) %>% 
  ggplot(aes(x=reorder(order, mean_mass), y=mean_mass))+geom_col()+  #reorder the order by mean_mass
  coord_flip()+
  scale_y_log10()
...
elephants %>% 
  ...
  labs(title = "Elephant Age vs Height",
       x= "Age",
       y= "Height") +
  theme(plot.title = element_text(size = rel(1.25), hjust = 0.5)) #move the title
```
...
```{r}
life_history %>% 
  ggplot(aes(x=gestation, y= mass, size=mass))+geom_point()+scale_y_log10()
```
