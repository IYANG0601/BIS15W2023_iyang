...
#data, mapping for the graph, aes for aesthetic, specify x and y
#what kind of graph is appropriate for this, box plot?
#+ is for adding layer
...
```{r}
iris %>% 
  ggplot(mapping =aes(x=Sepal.Length, y= Sepal.Width))+geom_point()
```
...
```{r}
homerange %>% 
  ggplot(mapping = aes(x=log10.mass, y= log10.hra))+geom_point()
```
...
```{r}
#big and messy so use this
homerange %>% 
  ggplot(mapping = aes(x=log10.mass, y= log10.hra))+geom_jitter()
```

To add a regression (best of fit) line, we just add another layer.
```{r}
homerange %>% 
  ggplot(mapping = aes(x=log10.mass, y= log10.hra))+geom_point()+ geom_smooth(method = lm, se= T)
```

### Practice
1. What is the relationship between log10.hra and log10.preymass? What do you notice about how ggplot treats NA's?
```{r}
#just removes the NAs automatically, just use the  na.rm = T for this
#se for standard error
homerange %>% 
  ggplot(mapping = aes(x=log10.preymass, y= log10.hra,))+geom_point(na.rm = T)+ geom_smooth(method = lm, se= T,  na.rm = T)
```
...
```{r}
#present data table and then follow up with plot for visual, no y axis as its just the count
homerange %>% 
...
homerange %>% 
  ...
  ggplot(aes(x=common.name, y=log10.mass))+ #one has to be categorical
  geom_col() 
```
...
homerange_mammals %>% 
  top_n(-10, log10.mass) %>% 
  ggplot(aes(x=species, y=log10.mass))+geom_col()+coord_flip()
