...
```{r, eval=F, echo=T}
#first line is to select inputs
ui <- fluidPage(
    selectInput("x", "Select X Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm") #selected refers to where drop down starts
)
...
#once user input is set up, need to tell how to build plot, with input functions, need the dollar sign to connect back
server <- function(input, output) {
  reactive({input$x
            input$y})
}
...
```{r}
penguins %>% 
  ggplot(aes(x=bill_depth_mm, y= bill_length_mm))+geom_point(na.rm = T)
```
...
server <- function(input, output) {
  output$plot <- renderPlot({
    ggplot(data=penguins, aes_string(x=input$x, y=input$y))+geom_point()
  })
...
```{r}
#only changing the fill variable
ui <- fluidPage(
