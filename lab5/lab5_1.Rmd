...
```{r}
(mammals)
#order family and genus would be better represented as factors
```
```{r}
mammals %>% mutate_if(is.character, as.factor)
#now all characters are factors, no need to do individually
```
...
fish %>%
  ...
  filter(lakeid == "AL")
#allows me to feed one into next function
#shift control m
```
...
```{r}
fish %>% 
  filter(lakeid %in% c("AR", "AL")) %>% 
  filter(between(radii_length_mm, 2, 4))
  
#could use filter(lakeid == "AL" | lakeid == "AR")
# | is or and & is and, have to use or because cant be from both AR and AL at same time
```
...
it is sort of like copy and paste
```{r}
mammals %>% 
...
mammals %>% 
  ...
  arrange(newborn)
```
make new column new_bown new 
if -999 in new, replace with NA
if normal, just transpose
## Practice
