...
new data set with variables of interest
```{r}
select(fish, "lakeid", "annnumber")
```

To add a range of columns use `start_col:end_col`.
use the names of the columns, not their numbers
```{r}
select(fish, start_(2):end_(5))
```
pulls at specifc values
```{r}
select(fish, fish_id:length)
```

The - operator is useful in select. It allows us to select everything except the specified variables.
can be more convenient to remove rather than list
```{r}
...
There are no NAs.
They put -999 instead of NA for some reason.
```{r}
...
```{r}
rename(mammals, "genus" = "Genus", "afr" = "AFR", "max_life" = "max. life")
```
...
```{r}
select(mammals,"species", "mass", "genus")
